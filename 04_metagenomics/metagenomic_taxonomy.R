...
packages_to_load <- c(
  "tidyverse","janitor","readr", "reshape2", "dplyr", "pheatmap", "qiime2R",
  "phyloseq", "this.path", "magrittr", "vegan", "pairwiseAdonis"
)
...
#remove week 9
asv16s3tga.f <-asv16s3tga[,c(1,2,3,7,8,9,13,14,15)]

asv16s3tgao <- as.matrix(asv16s3tga.f[order(rowSums(asv16s3tga.f),decreasing = T),])
asv16s3tgaop <- asv16s3tgao[c(1:10),]
other <- colSums(asv16s3tgao[c(11:nrow(asv16s3tgao)),])
asv16s3tgaopo <- rbind(asv16s3tgaop, other)

ave.gen_gg <- as.data.frame(asv16s3tgaopo)
ave.gen_gg <- adorn_percentages(ave.gen_gg,, 1:9, denominator="col")
other <- ave.gen_gg[11,]
ave.gen_gg <- ave.gen_gg[-11,]
ave.gen_gg <- ave.gen_gg[order(rowSums(ave.gen_gg),decreasing = F),]
ave.gen_gg <- rbind(other, ave.gen_gg)
ave.gen_gg[ "Genus" ] <- rownames(ave.gen_gg)
ave.gen_gg$Genus <- factor(ave.gen_gg$Genus, levels = ave.gen_gg$Genus)
...
filtered <- gather_table
filtered <- filtered[rowSums(filtered) != 0, ]
filtered <- filtered[, grepl("WK1", colnames(filtered))]
filtered <- as.data.frame(filtered)
filteredt<- t(filtered)
metag_dist <- vegdist(filteredt)
metaday1 <- metadata %>% filter(day == 1)
pw.metag <- adonis2(metag_dist ~ treatment, data=metaday1)
pw.metag 

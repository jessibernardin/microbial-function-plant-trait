...
packages_to_load <- c(
  ...
  "caret", "DT", "pairwiseAdonis", "brms", "reshape2", "gganimate", "gapminder",
  ...
  "readr", "tidyr", "viridis", "reshape", "forcats", "pheatmap", "this.path", "microbiome", "stats"
)
...
#### Tree #### 
asv16s.tree <- read.tree("tree_sepp_filt.nwk")
print(asv16s.tree$tip.label)
print(length(asv16s.tree$tip.label))#2633
...
head(asv16s.tree$tip.label, n=20)
...
asv16s.tax3raw <- cbind(ASV=row.names(asv16s.tax2raw),asv16s.tax2raw)
taxa_names(asv16s.tax3raw)<- asv16s.tax3raw$ASV

tree.16s.root.raw <-root(asv16s.tree, "ASV1268")## root with an archaeon
...
row_sums <- rowSums(otu_table(asv16s.physeq2))
nonzero_rows <- row_sums != 0
asv16s.physeq2 <- prune_taxa(nonzero_rows, asv16s.physeq2)#565 taxa and 144 samples
#935, 217
...
asv16s.physeq1all <-merge_phyloseq(phyloseq(r.OTU.16s),r.SAM.16s,r.TAX.16s,r.new_tree)
asv16s.physeq1all 
asv16s.physeq1 <- subset_samples(asv16s.physeq1all, treatment != "ACM")
asv16s.physeq1 <- subset_samples(asv16s.physeq1, treatment != "WATER")
#917 ASVs, 217 samples
row_sums <- rowSums(otu_table(asv16s.physeq1))
nonzero_rows <- row_sums != 0
asv16s.physeq1 <- prune_taxa(nonzero_rows, asv16s.physeq1)#565 taxa and 144 samples
#353, 217


#### Unweighted UniFrac ####
ps_treat_only_enzymena_uu <- subset_samples(asv16s.physeq1, chitinase_rate_uM_per_min != "NA")
row_sums <- rowSums(otu_table(ps_treat_only_enzymena_uu))
nonzero_rows <- row_sums != 0
ps_treat_only_enzymena_uu <- prune_taxa(nonzero_rows, ps_treat_only_enzymena_uu)
...
row_sums <- rowSums(otu_table(biomass_uu))
nonzero_rows <- row_sums != 0
biomass_uu <- prune_taxa(nonzero_rows, biomass_uu)
...
#unweighted stress=0.1746858
#weighted stress=0.1918879
## Plot NMDS
data.scores2 <- as.data.frame(scores(uu.nmds.16s.treat$points[,1:2]))
data.scores2$ID <- rownames(data.scores2)
phylo.meta <- sample_data(asv16s.physeq1)
md16s.filt <- as.data.frame(phylo.meta)
data_merge2 <- left_join(data.scores2, md16s.filt, by = c("ID"))
...
print(length(unique(tax$Kingdom)))#1
print(length(unique(tax$Phylum)))#17
print(length(unique(tax$Class)))#27
print(length(unique(tax$Order)))#58
print(length(unique(tax$Family)))#86
print(length(unique(tax$Genus)))#134

#### ANCOMBC DIFFERENTIAL ABUNDANCE ####
# use non-rarified data
tse = mia::makeTreeSummarizedExperimentFromPhyloseq(asv16s.physeq2)
tse$treatment <- factor(tse$treatment, levels=c("CommB", "CommA", "CommC"))

output_all = ancombc2(data = tse, assay_name = "counts", tax_level = NULL,
                       fix_formula = "treatment",
                       rand_formula = "(1 | week)",
                       p_adj_method = "fdr", pseudo_sens = TRUE,
                       prv_cut = 0.3, group="treatment",
                       alpha = 0.05, n_cl = 3, verbose = TRUE,
                       global = TRUE, pairwise = TRUE)
output_all_res <- output_all$res
output_all_pair <- output_all$res_pair
output_all_res_sig <- subset(output_all_res, `diff_(Intercept)`==T | diff_treatmentCommA==T |
                              diff_treatmentCommC==T)
dim(output_all_res)#[1] 73 22
dim(output_all_res_sig)#[1] 38 22
write.csv(output_all_res_sig, "ancombc_significant.csv")
output_all_res_sig_filt <- output_all_res_sig[,c(1,3,4,6,7, 18, 19)]
write.csv(output_all_res_sig_filt, "ancombc_significant.csv")


df_lfc <- melt(output_all_res_sig_filt, id.vars = 'taxon', 
               measure.vars = c('lfc_treatmentCommA', 'lfc_treatmentCommC'), 
               variable.name = 'Treatment', value.name = 'LogFoldChange')

df_se <- melt(output_all_res_sig_filt, id.vars = 'taxon', 
              measure.vars = c('se_treatmentCommA', 'se_treatmentCommC'), 
              variable.name = 'Treatment', value.name = 'SE')

df_diff <- melt(output_all_res_sig_filt, id.vars = 'taxon', 
              measure.vars = c('diff_treatmentCommA', 'diff_treatmentCommC'), 
              variable.name = 'Treatment', value.name = 'SE')


df_lfc$variable <- gsub('lfc_', '', df_lfc$variable)
df_se$variable <- gsub('se_', '', df_se$variable)
df_diff$variable <- gsub('diff_', '', df_diff$variable)

# Merge the two data frames by taxon and Treatment
df_melt <- merge(df_lfc, df_se, by = c('taxon', 'variable'))
colnames(df_melt)[2] <- "treatment"
colnames(df_melt)[3] <- "lfc"
colnames(df_melt)[4] <- "se"
colnames(df_diff)[2] <- "treatment"

df_melt <- merge(df_melt, df_diff, by = c('taxon', 'treatment'))
...
tax_ancom <- as.data.frame(tax_table(asv16s.physeq2))
colnames(tax_ancom)[1] <- "taxon"

filtered_df_tax <- left_join(filtered_df,tax_ancom, by="taxon")
filtered_df_tax$name <- paste(filtered_df_tax$taxon, filtered_df_tax$Genus, sep = "_")

filtered_df_tax <- filtered_df_tax %>%
  arrange(-lfc) %>%
  mutate(name = factor(name, levels = unique(name)))

filtered_df_tax_filtered <- filtered_df_tax %>%
  filter(abs(lfc) > 1.5)

ggplot(filtered_df_tax_filtered, aes(x = name, y =lfc , color = treatment)) +
  geom_point(position = position_dodge(width = .5)) +  # Adjust width as needed
  geom_errorbar(aes(ymin = lfc - se, ymax = lfc + se), 
                width = 0.2, position = position_dodge(width = .5), alpha=.5) +  # Adjust width as needed
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_color_manual(values=c("#014b7a", "#44b7c2")) +
  theme_minimal() +
...
### PA
treatment_group <- "CommB"

# 1. Subset Data by Treatment
physeq_treatment <- subset_samples(asv16s.physeq1, treatment == treatment_group)

# 2. Identify ASVs Present in One Treatment
otu_table_treatment <- otu_table(physeq_treatment)
asvs_present <- rownames(otu_table_treatment)[rowSums(otu_table_treatment) > 0]

# 3. Check Absence in Other Treatments
# Replace 'other_treatments' with a list of other treatment groups
other_treatments <- c("CommA", "CommC")
for (treatment in other_treatments) {
  physeq_other <- subset_samples(asv16s.physeq1, treatment == treatment)
  otu_table_other <- otu_table(physeq_other)
  asvs_present <- setdiff(asvs_present, rownames(otu_table_other)[rowSums(otu_table_other) > 0])
}

# ASVs present in 'treatment_group' and absent in all other treatments
asvs_present_unique <- asvs_present

# Optionally, you can examine the list of ASVs present in 'treatment_group' and absent in all other treatments
print(asvs_present_unique)
